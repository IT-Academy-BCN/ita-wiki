...
beforeAll(async () => {
  ...
  testTopic = await prisma.topic.findUnique({ where: { slug: 'testing' } }) as Topic;

  testUser = await prisma.user.findUnique({ where: { dni: testUserData.user.dni } }) as User;
  ...
  const resources = await prisma.$transaction(resourceData.map((resource) => prisma.resource.create({
    data: resource
  })));
...
afterAll(async () => {
  ...
  await prisma.topicsOnResources.deleteMany({ where: { topicId: testTopic.id } })
...
describe('GET /v1/resources/topic/:topicId', () => {
  ...
  test('should fail if topic ID does not exist in database ', async () => {
    ...
    const response = await supertest(server)
      .get(`${baseUrl}/this-topicId-does-not-exist`);
...
describe('GET /v1/resources/topic/slug/:slug', () => {
  ...
  test('should respond with OK status if topic slug exists in database and return an array of resources associated with the topic slug  ', async () => {
    ...
    test('should fail if topic slug does not exist in database ', async () => {
      ...
      const response = await supertest(server)
      .get(`${baseUrl}/this-topicSlug-does-not-exist`);
