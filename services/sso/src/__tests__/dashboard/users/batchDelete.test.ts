...
describe('Testing dashboard delete endpoint', () => {
  ...
  it('should succeed if the same data is provided twice but keeping the first deletedAt value', async () => {
    let deletedAt1 = await db('user')
      .select('deleted_at')
      .whereIn('id', [userToDeleteId, userToBeBlockedId])
    expect(deletedAt1[0].deleted_at).toBe(null)
    expect(deletedAt1[1].deleted_at).toBe(null)
    const ids = [userToDeleteId, userToBeBlockedId]
    ...
    deletedAt1 = await db('user')
      .select('deleted_at')
      .whereIn('id', [userToDeleteId, userToBeBlockedId])

    expect(deletedAt1[0].deleted_at).toContain(Date)
    expect(deletedAt1[1].deleted_at).toContain(Date)
    const response2 = await supertest(server)
      .delete(`${route}/`)
      .set('Cookie', [authAdminToken])
      .send({ ids })
    expect(response2.status).toBe(204)
    const deletedAt2 = await db('user')
      .select('deleted_at')
      .whereIn('id', [userToDeleteId, userToBeBlockedId])
    expect(deletedAt1[0].deleted_at).toEqual(deletedAt2[0].deleted_at)
    expect(deletedAt1[1].deleted_at).toEqual(deletedAt2[1].deleted_at)
  })
  it('should succeed if one of the provided ids exist', async () => {
    let deletedAt = await db('user')
      .select('deleted_at')
      .whereIn('id', [userToDeleteId])
